...
from New_baseline import CCT_Unet
...
def trainModels(dataset_tag,
    ...
    for j in range(1, repeat + 1):
        ...
        Exp = CCT_Unet.CCT(in_ch=input_dim, width=width, class_no=class_no)
...
def trainSingleModel(model,
    ...
    for epoch in range(num_epochs):
        ...
        for j, (image_aug1, image_aug2, labels, imagename) in enumerate(trainloader):
            ...
            output, out_main, outputs_side = model(image)
            ...
            if torch.max(labels) == 100.0:
                ...
                for side_output in outputs_side:
                    loss += nn.MSELoss(reduction='mean')(torch.sigmoid(out_main), torch.sigmoid(side_output)) + \
                            nn.MSELoss(reduction='mean')(torch.sigmoid(side_output), torch.sigmoid(out_main))
        ...
        with torch.no_grad():
            ...
            for i, (val_images_aug1, val_images_aug2, val_label, imagename) in enumerate(validateloader):
                ...
                val_outputs, _, __ = model(val_img)
    ...
    for model in all_models:
        ...
        with torch.no_grad():
            ...
            for ii, (test_images1, test_images2, test_label, test_imagename) in enumerate(testdata):
                ...
                test_outputs, _, __ = model(test_img)
