...
def prepare_data(new_size, scans_folder, labels_folder, save_folder, input_dim, train_amount_slices, case_no):
    ...
    if input_dim == 3:
        save_lo = save_folder + '/very_dense_' + str(case_no) + '_r' + str(new_size) + '_s' + str(train_amount_slices)
    elif input_dim == 1:
        save_lo = save_folder + '/very_dense_' + str(case_no) + '_r' + str(new_size) + '_s' + str(train_amount_slices)
    elif input_dim == 5:
        save_lo = save_folder + '/very_dense_' + str(case_no) + '_r' + str(new_size) + '_s' + str(train_amount_slices)
    ...
    if train_amount_slices == 10000:
        ...
        pre_process(new_size=new_size, all_cases_scans=train_withoutlabels_scans, all_cases_labels=train_withoutlabels_labels, save_folder=train_save_folder, input_dim=input_dim, train_amount_slices=10000, with_label=True, sample_mode='sparse')
        pre_process(new_size=new_size, all_cases_scans=train_withlabels_scans, all_cases_labels=train_withlabels_labels, save_folder=train_save_folder, input_dim=input_dim, train_amount_slices=10000, with_label=True, sample_mode='sparse')

    else:

        pre_process(new_size=new_size, all_cases_scans=train_withoutlabels_scans, all_cases_labels=train_withoutlabels_labels, save_folder=train_save_folder, input_dim=input_dim, train_amount_slices=10000, with_label=False, sample_mode='dense')
        pre_process(new_size=new_size, all_cases_scans=train_withlabels_scans, all_cases_labels=train_withlabels_labels, save_folder=train_save_folder, input_dim=input_dim, train_amount_slices=train_amount_slices, with_label=True, sample_mode='very_dense')

    pre_process(new_size=new_size, all_cases_scans=validate_scans, all_cases_labels=validate_labels, save_folder=validate_save_folder, input_dim=input_dim, train_amount_slices=10000, with_label=True, sample_mode='sparse')
    pre_process(new_size=new_size, all_cases_scans=test_scans, all_cases_labels=test_labels, save_folder=test_save_folder, input_dim=input_dim, train_amount_slices=10000, with_label=True, sample_mode='sparse')


def pre_process(new_size, all_cases_scans, all_cases_labels, save_folder, input_dim, train_amount_slices, with_label, sample_mode):
    ...
    for case_scan, case_label in zip(all_cases_scans, all_cases_labels):
        ...
        if sample_mode == 'dense':
            ...
            for slice_no in range(last_slice_no):
                ...
                scan, label = crop_slice(ct_scan, label_scan, left, right, top, bottom, input_dim, sample_position)
            ...
            for slice_no in range(last_slice_no):
                ...
                for ww in range(0, width, 20):

                    for hh in range(0, height, 20):
                        ...
                        if right >= width or bottom >= height:
                            ...
                            position_index = '_h' + str(hh) + '_w' + str(ww) + '_'

                            save_img_name = case_index + position_index + str(slice_no) + '_slice.tif'
                            full_save_img_name = os.path.join(case_patches_folder, save_img_name)

                            save_label_name = case_index + position_index + str(slice_no) + '_label.tif'
                            full_save_label_name = os.path.join(case_labels_folder, save_label_name)
...
if __name__ == '__main__':
    ...
    # workstation path:
    # images_folder = '/home/moucheng/projects_data/Pulmonary_data/CARVE2014/ctscans/fullannotations'
    # labels_folder = '/home/moucheng/projects_data/Pulmonary_data/CARVE2014/labels/fullannotations'
    # save_folder = '/home/moucheng/projects_data/Pulmonary_data/CARVE2014/others'

    images_folder = '/home/moucheng/projects_data/Pulmonary_data/CARVE2014/ctscans/fullannotations'
    labels_folder = '/home/moucheng/projects_data/Pulmonary_data/CARVE2014/labels/fullannotations'
    save_folder = '/home/moucheng/projects_data/Pulmonary_data/CARVE2014/miccai exps/'
