...
# ===================================================
from NewERFNet import NewERFAnet, constrained_loss

from NNUtils import sigmoid_rampup, exp_rampup

# =======================================================================================
# self supervision on encoder
# semi supervision on decoder
# consistency loss on unsupervised part of semi supervision
# an entropy consistency loss on unsupervised part of semi supervision as well
# =======================================================================================
...
def trainModels(dataset_tag,
    ...
    for j in range(1, repeat + 1):
        ...
        Exp = NewERFAnet(in_ch=input_dim, width=width, class_no=class_no, identity_add=self_addition)
...
def trainSingleModel(model,
    ...
    decoder = list(model.fpa3_smooth.parameters()) + list(model.fpa3.parameters()) + \
              list(model.fpa2_smooth.parameters()) + list(model.fpa2.parameters()) + \
              list(model.fpa1_smooth.parameters()) + list(model.fpa1.parameters()) + \
              list(model.fpa0_smooth.parameters()) + list(model.fpa0.parameters()) + \
              list(model.fna3_smooth.parameters()) + list(model.fna3.parameters()) + \
              list(model.fna2_smooth.parameters()) + list(model.fna2.parameters()) + \
              list(model.fna1_smooth.parameters()) + list(model.fna1.parameters()) + \
              list(model.fna0_smooth.parameters()) + list(model.fna0.parameters()) + \
              list(model.dconv_fp_last.parameters()) + list(model.dconv_fn_last.parameters())

    optimizer_encoder = torch.optim.AdamW(encoder, lr=learning_rate, betas=(0.9, 0.999), eps=1e-8, weight_decay=1e-5)
    optimizer_decoder = torch.optim.AdamW(decoder, lr=learning_rate, betas=(0.9, 0.999), eps=1e-8, weight_decay=1e-5)
    ...
    for epoch in range(num_epochs):
        ...
        for j, (images1, images2, images3, labels, imagename) in enumerate(trainloader):
            ...
            loss_entropy_student = (-prob_outputs_fp*torch.log(prob_outputs_fp)).mean()
            loss_entropy_teacher = (-prob_outputs_fn * torch.log(prob_outputs_fn)).mean()
            ...
            if torch.max(labels) == 100.0 and torch.min(labels) == 100.0:
                ...
                loss = alpha_current*loss + weight_regularisation*(loss_entropy_student - loss_entropy_teacher)**2
        ...
        # # gamma = exp_rampup(current=epoch, base=1.0, limit=100.0)
        # alpha_current = pow(2.712828, -5*((1-epoch/num_epochs)**2))
        # if alpha_current > 0.1:
        #     alpha_current == 0.1
        #
        # gamma = pow(1.05, epoch)
        # if gamma > 100.0:
        #     gamma == 100.0
