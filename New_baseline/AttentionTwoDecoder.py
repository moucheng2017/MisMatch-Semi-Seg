...
class AttentionDecoderUnet(nn.Module):

    # baseline:
    # same architecture, but outputs are directly attention

    def __init__(self, in_ch, width, class_no, identity_add=True):

        super(AttentionDecoderUnet, self).__init__()
        ...
        # self.self_constrained_econv3 = ContrastiveModule(in_ch=self.w4)
        # self.self_constrained_econv2 = ContrastiveModule(in_ch=self.w3)
...
class ContrastiveModule(nn.Module):
    ...
    def forward(self, x1, x2, x3):
        ...
        # Reshape tensors:
        x1_query = x1_query.view(b, c, h*w).permute(0, 2, 1).contiguous()
        # b x c x n
...
def constrained_loss(x1, x2, x3, pseudo_x1_a, pseudo_x1_b, pseudo_x2, pseudo_x3_a, pseudo_x3_b, normalisation='softmax'):
