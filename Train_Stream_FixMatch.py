...
from NNUtils import CustomDataset_FixMatch
...
def getData(data_directory, dataset_name, dataset_tag, train_batchsize, data_augment, cross_validation):
    ...
    if cross_validation is False:
        ...
        #
        train_dataset = CustomDataset_FixMatch(train_image_folder, train_label_folder, True, 3)
        #
        validate_dataset = CustomDataset_FixMatch(validate_image_folder, validate_label_folder, False, 3)
        #
        test_dataset = CustomDataset_FixMatch(test_image_folder, test_label_folder, False, 3)
        #
...
def trainSingleModel(model,
    ...
    for epoch in range(num_epochs):
        ...
        for j, (image_aug1, image_aug2, labels, imagename) in enumerate(trainloader):
            ...
            # image_aug1: strong augmentation
            # image_aug2: weak/no augmentation
            ...
            if torch.max(labels) == 100.0 and torch.min(labels) == 100.0:
                ...
                pseudo_label = (prob_outputs_aug2 > 0.7).float()

                if losstag == 'dice':

                    loss = dice_loss(prob_outputs_aug1, pseudo_label)

                elif losstag == 'ce':

                    loss = nn.BCEWithLogitsLoss(reduction='mean')(prob_outputs_aug1, pseudo_label)
