...
from NNLoss import dice_loss, mse, pseudo_features
...
# =======================================================================================
# self supervision on encoder and decoder
# self supervision on features after the last convolutional layer
# semi supervision on decoder and decoder
# consistency loss on unsupervised part of semi supervision
# anneal down inside of each iteration
# =======================================================================================
...
def trainModels(dataset_tag,
    ...
    for j in range(1, repeat + 1):
        ...
        Exp_name = 'ERFAnet16.0' + \
                   '_repeat_' + str(repeat_str) + \
                   '_augmentation_' + str(augmentation) + \
                   '_alpha_' + str(alpha) + \
                   '_lr_' + str(learning_rate) + \
                   '_epoch_' + str(num_epochs) + \
                   '_anneal_' + annealing_mode + '_at_' + str(annealing_threshold) + '_beta_' + str(annealing_factor) \
                   + '_gamma_' + str(gamma) \
...
def trainSingleModel(model,
    ...
    for epoch in range(num_epochs):
        ...
        for j, (images1, images2, images3, labels, imagename) in enumerate(trainloader):
            ...
            # =======================================
            outputs_fp1, outputs_fn1, outputs_fp1_, outputs_fn1_ = model(images1)
            outputs_fp2, outputs_fn2, outputs_fp2_, outputs_fn2_ = model(images2)
            outputs_fp3, outputs_fn3, outputs_fp3_, outputs_fn3_ = model(images3)

            outputs_fp1_side = nn.MaxPool2d(2)(outputs_fp1)
            outputs_fp2_side = nn.MaxPool2d(2)(outputs_fp2)
            outputs_fp3_side = nn.MaxPool2d(2)(outputs_fp3)
            outputs_fn1_side = nn.MaxPool2d(2)(outputs_fn1)
            outputs_fn2_side = nn.MaxPool2d(2)(outputs_fn2)
            outputs_fn3_side = nn.MaxPool2d(2)(outputs_fn3)

            outputs_fp1_side = nn.MaxPool2d(2)(outputs_fp1_side)
            outputs_fp2_side = nn.MaxPool2d(2)(outputs_fp2_side)
            outputs_fp3_side = nn.MaxPool2d(2)(outputs_fp3_side)
            outputs_fn1_side = nn.MaxPool2d(2)(outputs_fn1_side)
            outputs_fn2_side = nn.MaxPool2d(2)(outputs_fn2_side)
            outputs_fn3_side = nn.MaxPool2d(2)(outputs_fn3_side)

            pseudo_fp1_a, pseudo_fp1_b, pseudo_fp3_a, pseudo_fp3_b = pseudo_features(outputs_fp1_side, outputs_fp2_side, outputs_fp3_side)
            pseudo_fn1_a, pseudo_fn1_b, pseudo_fn3_a, pseudo_fn3_b = pseudo_features(outputs_fn1_side, outputs_fn2_side, outputs_fn3_side)

            # pseudo_fp1_a, pseudo_fp1_b, pseudo_fp3_a, pseudo_fp3_b = pseudo_features(outputs_fp1, outputs_fp2, outputs_fp3)
            # pseudo_fn1_a, pseudo_fn1_b, pseudo_fn3_a, pseudo_fn3_b = pseudo_features(outputs_fn1, outputs_fn2, outputs_fn3)

            self_constrained_loss_fp1 = mse(pseudo_fp1_a, pseudo_fp1_b)
            self_constrained_loss_fp3 = mse(pseudo_fp3_a, pseudo_fp3_b)
            self_constrained_loss_fn1 = mse(pseudo_fn1_a, pseudo_fn1_b)
            self_constrained_loss_fn3 = mse(pseudo_fn3_a, pseudo_fn3_b)

            self_constrained_loss = gamma*0.25*(self_constrained_loss_fn1 + self_constrained_loss_fn3 + self_constrained_loss_fp1 + self_constrained_loss_fp3)

            prob_outputs_fp = torch.sigmoid(outputs_fp2)
            prob_outputs_fn = torch.sigmoid(outputs_fn2)
        ...
        with torch.no_grad():
            ...
            for i, (val_images1, val_images2, val_images3, val_label, imagename) in enumerate(validateloader):
                ...
                val_outputs_fp, val_outputs_fn, _, __ = model(val_img2)
    ...
    for model in all_models:
        ...
        with torch.no_grad():
            ...
            for ii, (test_images1, test_images2, test_images3, test_label, test_imagename) in enumerate(testdata):
                ...
                test_outputs_fp, test_outputs_fn, _, __ = model(test_img2)
