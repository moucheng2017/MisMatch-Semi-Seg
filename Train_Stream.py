...
# from Baselines import DMNet_seg, DMNet_dis
...
def trainModels(dataset_tag,
    ...
    for j in range(1, repeat + 1):
        ...
        if network == 'Entropy_minimisation':

            Exp = UNet(in_ch=input_dim, width=width, class_no=class_no)

            Exp_name = 'EntropyMinimisation' + \
        ...
        elif network == 'MTASSUnet':

            Exp = MTASSUnet(in_ch=input_dim, width=width, class_no=class_no)
...
def trainSingleModel(model,
    ...
    for epoch in range(num_epochs):
        ...
        for j, (images1, images2, images3, labels, imagename) in enumerate(trainloader):
            ...
            if 'MTASSUnet' in model_name:
                # Multi-Task Attention-Based Semi-Supervised
                # Learning for Medical Image Segmentation
                # reference: https://arxiv.org/abs/1907.12303
                outputs, attention_outputs = model(images)

                prob_outputs = torch.sigmoid(outputs)
                attention_outputs = torch.sigmoid(attention_outputs)

                # calculate the foreground and background
                foreground = images*prob_outputs
                background = images*(1.0 - prob_outputs)
                reconstruction = torch.cat([foreground, background], dim=1)
                ...
                if torch.max(labels) == 100.0 and torch.min(labels) == 100.0:
                    ...
                    loss = nn.MSELoss(reduction='mean')(reconstruction, attention_outputs) + nn.MSELoss(reduction='mean')(attention_outputs, reconstruction)

                    loss = 0.5*alpha_current*loss
                ...
                # baseline: entropy minimisation when labels is not available
                # reference: https://papers.nips.cc/paper/2740-semi-supervised-learning-by-entropy-minimization.pdf
                outputs = model(images)
                prob_outputs = torch.sigmoid(outputs)

                if torch.max(labels) == 100.0 and torch.min(labels) == 100.0:
                    # print('training image without label')
                    image_no_label += 1

                    loss = -1*prob_outputs*torch.log(prob_outputs)
        ...
        with torch.no_grad():
            ...
            for i, (val_images1, val_images2, val_images3, val_label, imagename) in enumerate(validateloader):
                ...
                # elif 'DMNet' in model_name:
                #
                #     val_outputs1, val_outputs2, outputs1_dis, outputs2_dis, label_dis = model(val_img, val_label)
                #     val_outputs = (val_outputs1 + val_outputs2) / 2
                #     val_class_outputs = torch.sigmoid(val_outputs)
    ...
    for model in all_models:
        ...
        with torch.no_grad():
            ...
            for ii, (test_images1, test_images2, test_images3, test_label, test_imagename) in enumerate(testdata):
                ...
                # elif 'DMNet' in model_name:
                #
                #     test_outputs1, test_outputs2, outputs1_dis, outputs2_dis, label_dis = model(test_img, test_label)
